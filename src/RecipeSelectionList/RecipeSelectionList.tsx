...
export default function RecipeSelectionList({ recipe, setSelectedRecipe, selectedRecipe }:
    {
        recipe: MyFormValues[],
        setSelectedRecipe: Dispatch<SetStateAction<{ count: number; recipe: string[]; }>>,
        selectedRecipe: {
            count: number, recipe: string[]
        }
    }) {

    const [generatedShoppingList, setGeneratedShoppingList] = useState(initialShoppingData)
        ...
        <>
            {!isFetched ? (
                <>
                    ...
                    <button className="GenerateListButton" onClick={shoppingList} disabled={loading}>
                        <RecipeDetails
                            icon={RiAiGenerate}
                            content={loading ? "Generating with AI..." : `Generate Shopping List (${selectedRecipe.count} recipes)`}
                            color="white"
                            size="1.2rem"
                        />
                    </button>
